...
max_dist = 180 # max distance to keep the target object (in cm)
min_dist = 120 # Minimum distance the stereo setup can measure (in cm)
sample_delta = 10 # Distance between two sampling points (in cm)

Z = max_dist 
Value_pairs = []

disp_map = np.zeros((800,720,3))
...
cv2.resizeWindow('disp',800,720)
cv2.namedWindow('left image',cv2.WINDOW_NORMAL)
cv2.resizeWindow('left image',800,720)
...
while True:
	...
	if retL and retR:
		...
		f_pixel=879.4197998
		baseline= 9.28874
		depth_map= (f_pixel*baseline)/disparity
